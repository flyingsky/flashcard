...
(function($){
    var strings = {
        strConversion: {
            ...
            // Signed integer to time format hhhhhh:mm:ss
            t: function(input, args){
                var num = parseInt(input);
                if(isNaN(num)) {
                    console.log('format:t error, NaN, ' + input);
                    return '0';
                }
                ...
                var h = num;		// num % 24;
                var d = 0;			// (num - h) / 24;
                var formatStr = '{1:d}:{2:02d}:{3:02d}';
                if (d > 0) {
                    formatStr = '{0}d ' + formatStr;
                }
                return $.format(formatStr, d, h, m, s);
            },
            D: function(input, args){
              input = this.__makeSureDate(input);
              return $.format('{0:d}-{1:02d}-{2:02d}', input.getFullYear(), (input.getMonth() + 1), input.getDate());
            },
            H: function(input, args){
              input = this.__makeSureDate(input);
              return $.format('{0:02d}:{1:02d}', input.getHours(), input.getMinutes());
            },
            ...
            __makeSureDate: function(input) {
              return $.type(input) == 'date' ? input : new Date(input);
            }
        },
